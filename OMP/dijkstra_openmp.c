...
void generate_random_graph(int V, int *adjacency_matrix) {
    srand(0); // Sử dụng hạt giống cố định để kết quả ổn định
    for (int i = 0; i < V; i++) {
        for (int j = 0; j < V; j++) {
            if (i != j) {
                adjacency_matrix[i * V + j] = rand() % 10 + 1; // Trọng số từ 1 đến 10
            } else {
                adjacency_matrix[i * V + j] = 0; // Không có cạnh tới chính nó
            }
        }
    }
}
...
void dijkstra_parallel(int V, int *adjacency_matrix, int source, int *distances) {
    ...
    for (int count = 0; count < V - 1; count++) {
        ...
        // Tìm đỉnh có khoảng cách nhỏ nhất chưa thăm (song song)
